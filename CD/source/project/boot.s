...
@ 
@ Info:			This file is originally from Jeff 
@			Frohweins crt0.s. If he or anybody else has a problem with this
@			then just tell me and I'll replace it;
@
@ 
@ -------------------------------------------

@********************************************************************
@*   crt0.S v1.25 by Jeff Frohwein                                  *
@********************************************************************
...
@ your C code in your main project file:
...
@ Comment out the following line to disable multiple-interrupts-
@ at-once support. If you have several interrupts where one can
@ occur while another is being serviced then you need to enable
@ this option. Disabling it allows slightly smaller/faster code.
...
@ must have the cart interrupt enabled for this to work.
...
@ Jump to user code
        ...
        ldr     r1,=start_vector
        mov     lr,r1            @ Set start_vector as return address
...
intr_main:                               @ Multiple interrupts support
        mov     r3, #0x4000000           @ REG_BASE
        ldr     r2, [r3,#0x200]!         @ r3 = IE : r2 = IF|IE
...
@ -------------------------------------------
@ End of crt0.S v1.25 by Jeff Frohwein                                  
@ -------------------------------------------
